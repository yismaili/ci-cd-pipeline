                ...
                steps {
                    script {
                        try {
                            ...
                            def backendTags = sh(script: "docker images --format '{{.Repository}}:{{.Tag}}' | grep '192.168.100.75:8585/ci-cd/backend/backend-' || true", returnStdout: true).trim().split('\n').findAll { it }
                            def frontendTags = sh(script: "docker images --format '{{.Repository}}:{{.Tag}}' | grep '192.168.100.75:8585/ci-cd/frontend/frontend-' || true", returnStdout: true).trim().split('\n').findAll { it }
