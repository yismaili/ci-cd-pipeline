...
pipeline {
    ...
    parameters {
        ...
        string(name: 'NEXUS_ARTEFACT_CREDENTIALS', defaultValue: '', description: 'Nexus artifact credentials')
    }
        ...
        stage('Checkout') {
            steps {
                script {
                    ...
                    echo "Using script path: ${scriptPath}"
                    checkout([
                        $class: 'GitSCM',
                        branches: [[name: "${params.BRANCH}"]],
                        userRemoteConfigs: [[url: "${env.REPO_URL}", credentialsId: "${params.GIT_CREDENTIALS_ID}"]]
                    ])
                }
            }
        }
